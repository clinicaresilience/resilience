...
export function CalendarBooking({ profissionalId, profissionalNome, onBookingSelect }: CalendarBookingProps) {
  ...
  const fetchAgenda = async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/profissionais/agenda?profissionalId=${profissionalId}`)
      if (res.ok) {
        const data = await res.json()
        const slots: AgendaSlot[] = (data.slots || []).map((slot: any) => {
          // converter diaSemana em data real (próxima ocorrência)
          const hoje = new Date()
          const diaSemanaAtual = hoje.getDay()
          let diff = slot.diaSemana - diaSemanaAtual
          if (diff < 0) diff += 7
          const dataSlot = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate() + diff)
          return {
            ...slot,
  ...
  const events = useMemo(() => {
    ...
    return Object.entries(slotsByDate).map(([date, slots]) => ({
      ...
      allDay: true,
      resource: { date, slotsCount: slots.length, slots }
    }))
  }, [agendaSlots])
  ...
  const formatDate = (date: Date) => moment(date).format('dddd, DD [de] MMMM [de] YYYY')
