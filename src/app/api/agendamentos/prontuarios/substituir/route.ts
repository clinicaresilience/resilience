...
// PUT /api/agendamentos/prontuarios/substituir
// Substituir o PDF de um prontuário
export async function PUT(req: NextRequest) {
  try {
    ...
    const prontuarioId = formData.get('prontuarioId') as string;
    const arquivo = formData.get('arquivo') as File;

    if (!prontuarioId || !arquivo) {
      return NextResponse.json(
        { error: "prontuarioId e arquivo são obrigatórios" },
        { status: 400 }
      );
    }
    ...
    console.error("Erro ao substituir PDF do prontuário:", error);
    const errorMessage = error instanceof Error ? error.message : String(error);
    return NextResponse.json(
      { error: "Erro ao substituir PDF do prontuário", detail: errorMessage },
      { status: 500 }
    );
  }
}
